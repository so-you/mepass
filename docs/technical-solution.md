...
| SQLite 驱动 | sql.js | 纯 JS 实现，无需 native 编译，同步 API |
| 加密 | Node crypto AES-256-GCM | 加密 password、apikey、note |
| 数据密钥生成 | crypto.randomBytes(32) | 初始化时生成随机数据密钥 |
| 主密码 KDF | scrypt | 从用户主密码派生密钥加密密钥 |
| 系统密钥保存 | 平台 shell 命令 | macOS: `security` CLI, Linux: `secret-tool`, Windows: PowerShell CliXml |
| 本地密钥兜底 | 0600 权限 key file | 系统钥匙串不可用时使用 |
...
- `mepass.db` 保存业务数据。
- `config.json` 保存非敏感配置。当前实际配置存储在 `vault_meta` 表中。
...
| Windows | `win32` | PowerShell CliXml 文件加密 | `%APPDATA%/mePass/` |

实现要求：

- `platform/paths.ts` 必须根据 `process.platform` 返回数据目录、数据库路径、配置路径和 `vault.key` 路径。
- `key-store.ts` 优先使用平台 shell 命令写入系统钥匙串（macOS Keychain、Linux Secret Service、Windows PowerShell CliXml）；不可用时自动 fallback 到 `vault.key`。
...
1. 优先使用平台 shell 命令写入系统钥匙串。
2. 钥匙串不可用或写入失败时，写入 `vault.key`。
3. `vault.key` 内容为 base64 编码的 32 字节密钥。
4. `vault.key` 文件权限必须设置为 `0600`。
5. `vault_meta` 表记录密钥来源。
...
4. 如果读取成功，使用 `key_check` 验证密钥是否匹配当前数据库。
5. 验证通过则直接执行命令。
6. 验证失败（可能是旧数据库迁移），清除本地密钥，进入主密码恢复流程。
7. 如果密钥不存在且数据库未初始化，提示执行 `mepass init`。
8. 如果密钥不存在但数据库已存在，提示用户输入主密码。
9. 使用主密码解密 `vault_meta` 中的 `encryptedDataKey`。
10. 解密成功后将 dataKey 保存到本机 key-store。
11. 继续执行命令。
...
1. 查询候选记录。
2. 多条结果时以表格列表展示，提示用户用 short_id 精确查询。
3. 更新 `last_accessed_at`。
4. 敏感字段默认显示为 `••••••`。
5. 传 `--reveal` 时解密并输出适用敏感字段明文。
6. `--json` 默认不含敏感字段，配合 `--reveal` 时输出。
7. 传 `--copy` 时解密对应字段并写入剪贴板。
8. 复制后启动 60 秒延迟清理剪贴板（仅当内容未被替换时）。
...
- AES-256-GCM 加密工具。
- `init/add/list/get/edit/delete/status/backup` 命令。
...
10. 实现 `add`。
11. 实现 `list` 和查询条件。
12. 实现 `get`、`--reveal`（默认隐藏敏感字段）、`--copy`。
13. 实现 `edit`。
14. 实现 `delete`。
15. 实现 `status`。
16. 补齐测试和 README 示例。
